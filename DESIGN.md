...
# PART II - ACUMEN DESIGN INTENT

**Everything above this rule is generated: `npx getdesign@latest add cohere`, run 08-Aug-2026,
unedited. Everything below it is ours.** The architect's 07-Aug-2026 design law (recorded
verbatim in QUESTIONS.md) is in two halves - the cohere system, *and* deliberate, considered
design judgment. The half above is the system. This is the judgment, written down so a reviewer
can hold a screen against it instead of against taste.

## Who reads this, and when

**One person. One trader. Once a day, at 09:00, on one screen he is already sitting in front
of.** He is not technical. He is not reading; he is *checking*. He has a chart open in another
window and this tool is the thing that tells him to look at it.

That single sentence decides almost everything below. There is no second user, no team view, no
mobile session, no returning-after-a-week onboarding problem. There is no engagement to win: he
already opened it. The screen's only job is to be **read correctly in two seconds from four feet
away**, twenty or thirty times across a morning, and to be **right**.

The morning has exactly four moments, and the design serves them in this order:

| Time | What he needs to know | What the screen owes him |
|---|---|---|
| before 09:15 | is the tool ready, and what is today's bias per stock | one line: ready / not ready, and why not |
| 09:15-11:14 | is it still collecting | quiet. A progress reading, nothing more |
| 11:15 | the POCs are fixed and the day's watchlist exists | the armed stocks, separated from everything else |
| 11:30-15:15 | **has anything fired** | the alert, with the four numbers he trades on |

## What matters

**Glanceability over completeness.** Everything the tool knows is available; almost none of it
is on the first screen. A number that does not change what he does in the next fifteen minutes
does not get first-screen space. The armed list is short by construction (a handful of ~210) and
it is the only thing above the fold.

**State at a glance, carried by exactly one channel.** Every stock is in exactly one of seven
states and the state is legible without reading the word: position on the page first, then
colour, then the word. A stock never appears twice, never in two states, and never in an
unnamed one - the honest states include `refused` and `skipped`, because a stock that could not
be evaluated is information, not an absence.

**Zero decoration that costs comprehension.** No shadows, no gradients, no icons that repeat a
word already on the screen, no chart on the dashboard, no animation except the one that marks a
NEW alert. cohere's own instruction - *"Do not make every section card-based; Cohere often uses
unframed rows, rules, and open space"* - is the right one here for a reason it did not intend:
a border around every row is 210 rectangles competing with the four that matter.

**Silence is a state, and it must look like one.** The safety rail is that the screener
degrades to silence and never to a wrong alert (CONTEXT section 1 R4, and this chunk's own
tripwires). A silent screen therefore has two meanings - *nothing has fired* and *I am broken* -
and they must never be confusable. The failure banner is the one element permitted to occupy the
full width at the top of the page, in `error` red, and it is present or it is absent; it is
never a small grey note.

**Nothing on this screen is invented.** Every figure is a stored or fetched value the recording
carries; there are no placeholder rows, no sample stocks, no illustrative numbers. cohere's
iteration guide says *"placeholder product frames are better than invented product content"* -
in a trading tool that is not a preference, it is the difference between a mock and a lie.

## How the tokens are used (and the ones we do not use)

The palette is cohere's, unchanged. What is ours is the **assignment**: colour on this screen
means *state* and means nothing else. There is no brand colour, no accent for emphasis, no
coloured heading.

| State | Token | Why this one |
|---|---|---|
| `waiting` | `muted` #93939f on `canvas` | The default and the majority. It must recede - it is the absence of news |
| `armed` | `coral` #ff7759 | The system's only warm colour, and cohere reserves it for taxonomy rather than for CTAs. Here it is the single "watch this" mark: warm enough to find at a glance, not the alarm colour |
| `triggered` | `deep-green` #003c33 filled, `on-dark` text | The system's highest-contrast band, spent on the one event that asks him to act |
| `in-trade` | `deep-green` rule on `pale-green` #edfce9 | Deliberately the same family as `triggered`, one step quieter: the position is open, nothing is owed |
| `exited` | `ink` #212121 on `canvas`, no fill | Settled. Historical. Present for the log, not for the eye |
| `refused` | `error` #b30000 as a 2px left rule, never a fill | A stock the gates or the data refused. Visible, and structurally incapable of out-shouting an armed row |
| `skipped` | `slate` #75758a with the same left rule | Missed its data window (CONTEXT 4.4) and will be re-polled. Distinct from `refused`: one is a verdict, the other is a retry |

**Type.** `mono-label` (CohereMono 14) for every symbol and every price, because the whole value
of the table is that the digits form a scannable column - proportional numerals in a price
column are a comprehension cost with no benefit. `body` 16 for prose, `caption` 14 for the
timestamp on an alert, `card-heading` 32 for the one clock/status line. **No hero or product
display type anywhere**: 96px is a marketing gesture and this is an instrument panel. cohere's
own principle - *"Use massive type sparingly"* - taken to its conclusion for a screen with no
marketing job at all.

**Space.** The 8px scale, `xl` (24px) between state groups and `lg` (16px) inside them, so the
grouping is legible before a single word is read. `xxl`/`section` are marketing rhythm and are
not used: vertical drama costs him rows.

**Radius.** `xs` (4px) on the state chips and nothing else. The dashboard has no media, so it
has no need for cohere's signature 22px card. Using it anyway would be the tell - the generic
output the ruling forbids.

## The test

The architect's sentence is the acceptance criterion and it is not rhetorical: **would a
designer sign this?** Concretely, for any screen in this tool:

1. Can a stranger name every stock's state in two seconds without reading a legend?
2. Is every colour on the screen carrying state, and nothing else?
3. If the tool were broken, would this screen look different from a quiet market?
4. Is there a single element that could be deleted with no loss of meaning? If yes, delete it.
