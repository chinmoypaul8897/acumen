...
must come back byte-identical, in ALL THREE files (REVIEW_8 finding C2).
...
shards for the six days it replays, the 1-minute lake and the daily store, and writes exactly
three files inside the repo (the pack, its JSON companion, and `docs/reports/points_by_symbol.md`
-- the full per-stock points table page 7 shows both ends of). The implementation lives in the
package at `src/acumen/trader_pack.py`; this file is the launcher, with the same bare-clone
