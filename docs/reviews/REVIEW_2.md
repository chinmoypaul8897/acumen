...
|---|---|---|---|
| F1 | **CLOSED** | chunk-3 prep (2026-07-24) | `_warm_up_cookies` takes the caller's `min_interval` and passes it to `_throttle`. The pin was rewritten as `test_the_cookie_warm_up_honors_the_callers_pacing`, which now asserts every wire gap >= 2.0s on the download path. |
| F2 | **CLOSED** | chunk-3 prep (2026-07-24) | `fetch_binary` decodes through `_decode_binary`, which raises `_Retry` for a body that opens as HTML — the same policy `fetch_json` applies to an undecodable 200. Measured: both paths now spend 4 attempts on an identical bot-shield body. A non-HTML corrupt body still fails fast (new test), and a good ZIP is still accepted on attempt 1 (new test). |
