...
- Remote: origin = the GitHub mirror — PUBLIC since the architect's ruling of 18-Aug-2026 (portfolio and demonstration only; all rights reserved). EVERY session's final end-of-session duty is: `git push origin main --tags`, and the session's final report block must state the pushed SHA. The remote is a backup mirror ONLY: never commit, edit, or merge via the GitHub web UI; never force-push; if local and remote ever diverge, STOP and ask the architect. If a session dies before pushing, the operator pushes manually before starting the next session. Still zero collaborators: nobody but the owner ever gets write access.
- **The repo is PUBLIC, so every rule above binds harder, not softer.** Every push is world-readable, permanent, and mirrored by strangers within minutes — nothing pushed can be quietly unpushed. Secrets never enter git (rule 4: never print, log, echo, or commit `.env`); no AI attribution anywhere in the history; clean, atomic commits with honest messages. Visibility changes the cost of a slip, never the standard.
